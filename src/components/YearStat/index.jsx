...
const YearStat = ({ year, onClick }) => {
  ...
  let timmmy_total_meters_avail = 0;
  let timmmy_total_seconds_avail = 0;
  runs.forEach((run) => {
    sumDistance += run.distance || 0;
    if (run.average_speed) {
      pace += run.average_speed;
      timmmy_total_meters_avail += run.distance || 0;
      timmmy_total_seconds_avail += (run.distance || 0) / run.average_speed;
    } else {
      paceNullCount++;
    }
  ...
  const avgPace = formatPace(timmmy_total_meters_avail / timmmy_total_seconds_avail);
